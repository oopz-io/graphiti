...
def _extract_value_from_protobuf(value: Any) -> Any:
    ...
    if isinstance(value, (str, int, float, bool)):
        return value
    elif value is None or (hasattr(value, 'WhichOneof') and value.WhichOneof('kind') == 'null_value'):
        return None
...
def _format_spanner_params(kwargs: dict[str, Any]) -> tuple[struct_pb2.Struct, dict[str, types.Type]]:
...
class SpannerDriver(GraphDriver):
    ...
    def __init__(
        ...
        self.client = spanner_v1.SpannerAsyncClient(
            ...
            try:
                ...
                async for partial_result in await self.client.execute_streaming_sql(request):
                    ...
                    if field_names is None and partial_result.metadata:
                        field_names = [field.name for field in partial_result.metadata.row_type.fields]
                    
                    # Process the result values
                    if field_names:
                        num_fields = len(field_names)
                        all_values = [_extract_value_from_protobuf(val) for val in partial_result.values]
    ...
    async def build_indices_and_constraints(self, delete_existing: bool = False) -> None:
        ...
        if delete_existing:
            ...
            logger.warning("delete_existing=True is not supported for Spanner driver. Schema will be created if it doesn't exist.")
        
        await self.initialize_schema()

    async def execute_query(self, cypher_query_: LiteralString, **kwargs: Any) -> tuple[list[dict[str, Any]], None, None]:
        ...
        try:
            ...
            found_write_keywords = [kw for kw in write_keywords if re.search(r'\b' + kw + r'\b', query_upper)]
            
            is_read_query = (
                query_upper.startswith('SELECT') or 
                (query_upper.startswith('GRAPH') and 'MATCH' in query_upper and not found_write_keywords)
            )
            ...
            if is_read_query:
                ...
                async for partial_result in await self.client.execute_streaming_sql(request):
                    ...
                    if field_names:
                        ...
                        rows.extend([_extract_value_from_protobuf(val) for val in partial_result.values])              
    ...
    async def close(self) -> None:
        ...
        # Spanner client doesn't need explicit cleanup
        pass
